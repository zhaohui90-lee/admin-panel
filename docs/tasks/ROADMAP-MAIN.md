...
**Tasks**:
- [ ] 2.1 侧栏背景改为 `#0f1729`，右侧 1px 分隔线 `rgba(255,255,255,0.06)`
- [ ] 2.2 顶部：医院 Logo 占位 + "运维中心" / "Maintenance Console"
- [ ] 2.3 导航项：4 项（运行状态/系统设置/硬件调试/故障申报），移除"交易日志"。高度 64px，左侧 4px 激活指示条，蓝色渐变激活态
- [ ] 2.4 替换图标：波形(运行)、齿轮(设置)、电路板(调试)、警告(申报)，20x20 SVG
- [ ] 2.5 底部操作员区：头像占位 + "管理员 Admin" + "锁定/注销"按钮 + "返回业务"按钮
- [ ] 2.6 移动端：移除底部 Tab Bar → 改为顶部标题栏 + 汉堡菜单按钮 + Drawer 抽屉侧边栏
- [ ] 2.7 内容区 fade-transform 过渡动画
