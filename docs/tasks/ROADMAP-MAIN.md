...
**Tasks**:
- [x] 5.1 定义 8 项硬件 Mock 数据：身份证读卡器、医保卡读卡器、银行卡读卡器、凭条打印机、报告打印机、发卡模块、扫码器、人脸识别相机
- [x] 5.2 网格布局：宽屏 4 列，竖屏 2 列
- [x] 5.3 硬件卡片：顶部 24x24 SVG 图标 + 状态点标签(ok/warning/error/checking/disabled)、中部硬件名称、底部状态详情
- [x] 5.4 交互："全部检测"按钮(依次 checking → 结果)、单项检测/复位按钮、error 状态红色渐变 + 3px 红左边框
- [x] 5.5 checking 状态动画(旋转/脉冲)
