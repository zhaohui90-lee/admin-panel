...
// --- Power actions ---
function onReloadPage()  { openConfirm('刷新业务页面', '确认重新加载业务页面？',       () => device.reloadPage())  }
function onRestartApp()  { openConfirm('重启客户端',   '确认重启自助机客户端程序？',   () => device.restartApp())  }
function onRebootOS()    { openConfirm('重启自助机',   '确认重启自助机？设备将重新启动。', () => device.rebootOS())    }
function onQuitApp()     { openConfirm('关闭客户端',   '确认关闭自助机客户端程序？',   () => device.quitApp())     }
function onShutdownOS()  { openConfirm('关闭自助机',   '确认关闭自助机？设备将完全断电。', () => device.shutdownOS())  }
</script>
...
<template>
  <div class="mx-auto max-w-5xl space-y-5 lg:space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-xl font-bold tracking-wide lg:text-2xl" style="color: var(--color-text-primary);">
          设备详情：
      ...
      <div class="flex items-center gap-2 self-start rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 lg:px-5 lg:py-2.5 lg:text-base">
    ...
    <!-- ── Status Cards ──────────────────────────────── -->
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-5" data-testid="status-cards">
      ...
      >
        <div class="flex items-center justify-between text-sm lg:text-base" style="color: var(--color-text-muted);">
          <span>今日交易数</span>
          <span class="opacity-60">(笔)</span>
        </div>
        <div class="text-4xl font-bold lg:text-5xl" style="color: var(--color-text-primary);">1,284</div>
      </div>
      ...
      >
        ...
        <div
          ...
          :style="{
            color: device.networkLatency < 50
              ? 'var(--color-success)'
    ...
    >
      <div class="border-b px-6 py-5 lg:px-8 lg:py-6" style="border-color: var(--color-border);">
        <h2 class="flex items-center gap-3 text-base font-bold lg:text-lg" style="color: var(--color-text-primary);">
          <span class="h-5 w-1 rounded-sm" style="background: var(--color-accent); opacity: 0.7;" />
          硬件自检状态
        </h2>
      </div>
        ...
        >
          ...
          <div class="flex items-center gap-4 lg:w-44">
            ...
            <span
              ...
              :style="{
                boxShadow: hw.status === 'online'  ? '0 0 8px rgba(34,197,94,0.6)'
                         : hw.status === 'warning' ? '0 0 8px rgba(245,158,11,0.6)'
                         : '0 0 8px rgba(239,68,68,0.6)'
              }"
            />
            <span class="text-base font-medium lg:text-lg" style="color: var(--color-text-primary);">{{ hw.name }}</span>
          </div>

          <!-- Detail / Progress -->
          <div class="flex-1 pl-8 lg:mx-6 lg:pl-0">
            <template v-if="hw.progress !== undefined">
              <div class="mb-2 text-sm lg:text-base" style="color: var(--color-text-muted);">{{ hw.detail }}</div>
              <div class="h-3 w-full overflow-hidden rounded-full lg:h-4" style="background: #e2e8f0;">
                <div
            ...
            <template v-else>
              <span
                ...
                :style="{ color: hw.status === 'error' ? 'var(--color-danger)' : 'var(--color-text-muted)' }"
              >
                {{ hw.detail }}
              </span>
            </template>
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4 p-6 sm:grid-cols-3 lg:grid-cols-5 lg:gap-5 lg:p-8" data-testid="action-buttons">
        <TouchButton data-testid="btn-reload-page" @click="onReloadPage">
          <svg class="h-5 w-5 lg:h-6 lg:w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>
          刷新页面
        </TouchButton>

        <TouchButton variant="warning" data-testid="btn-restart-app" @click="onRestartApp">
          <svg class="h-5 w-5 lg:h-6 lg:w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
          重启客户端
        </TouchButton>
        ...
        <TouchButton variant="danger" data-testid="btn-quit-app" @click="onQuitApp">
          <svg class="h-5 w-5 lg:h-6 lg:w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/></svg>
          关闭客户端
        </TouchButton>
        ...
        <div class="col-span-2 sm:col-span-1">
          <TouchButton variant="danger" data-testid="btn-shutdown-os" @click="onShutdownOS">
            <svg class="h-5 w-5 lg:h-6 lg:w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18.36 6.64a9 9 0 1 1-12.73 0"/><line x1="12" y1="2" x2="12" y2="12"/></svg>
            关闭自助机
          </TouchButton>
        </div>
      </div>
    ...
    >
      ...
      <div class="p-6 lg:p-8">
        ...
        <template v-if="!serverUrlEditing">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4" data-testid="url-display">
            <code
              class="flex-1 rounded-xl border px-4 py-3 font-mono text-sm lg:py-4 lg:text-base"
              style="background: var(--color-deep); border-color: var(--color-border); color: var(--color-text-secondary);"
            >
